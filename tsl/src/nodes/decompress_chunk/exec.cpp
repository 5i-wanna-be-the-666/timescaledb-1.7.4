...
decompress_chunk_begin(CustomScanState *node, EState *estate, int eflags)
{
	...
	Plan *compressed_scan =(Plan *) linitial(cscan->custom_plans);
...
decompress_chunk_create_tuple(DecompressChunkState *state)
{
	...
	while (true)
	{
		...
		{
			TupleTableSlot *subslot = ExecProcNode((PlanState *)linitial(state->csstate.custom_ps));
