...
gapfill_state_create(CustomScan *cscan)
{
	...
	state->subplan =(Plan*) linitial(cscan->custom_plans);
...
align_with_time_bucket(GapFillState *state, Expr *expr)
{
	...
	FuncExpr *time_bucket =(FuncExpr *) copyObject(linitial(cscan->custom_private));
	Datum value;
	bool isnull;
...
{
	CustomScan *cscan = castNode(CustomScan, state->csstate.ss.ps.plan);
	FuncExpr *func =(FuncExpr *) linitial(cscan->custom_private);
	FromExpr *jt =(FromExpr *) lthird(cscan->custom_private);
	ListCell *lc;
	Var *ts_var;
...
gapfill_begin(CustomScanState *node, EState *estate, int eflags)
{
	 ...
	 */
	FuncExpr *func =(FuncExpr *) linitial(cscan->custom_private);
	List *args =(List *) lfourth(cscan->custom_private);
	...
	for (i = 0; i < state->ncolumns; i++)
	{
		...
		{
			entry =(Node*) copyObject(list_nth(cscan->custom_scan_tlist, i));
...
fetch_subplan_tuple(CustomScanState *node)
{
	...
	while (true)
	{
		subslot = ExecProcNode((PlanState*)linitial(node->custom_ps));
...
gapfill_state_initialize_columns(GapFillState *state)
{
	...
	for (i = 0; i < state->ncolumns; i++)
	{
		tle =(TargetEntry *) list_nth(cscan->custom_scan_tlist, i);
		expr = tle->expr;
...
gapfill_is_group_column(GapFillState *state, TargetEntry *tle)
{
	ListCell *lc;
	CustomScan *cscan = castNode(CustomScan, state->csstate.ss.ps.plan);
	List *groups =(List *) lsecond(cscan->custom_private);
